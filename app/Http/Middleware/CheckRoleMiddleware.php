...
{
    /**
     * Maneja una solicitud entrante y verifica si el usuario tiene el rol de Empleado/Socio o Administrador.
     * 
     * Este método comprueba si el usuario autenticado tiene el rol de Empleado/Socio o Administrador.
    ...
    {
        if (!$request->user() || (!$request->user()->hasRole('Empleado') && !$request->user()->hasRole('Socio') && !$request->user()->hasRole('Administrador'))) {
            abort(403, 'Acceso denegado. Se requiere rol de Empleado/Socio o Administrador.');
        }
